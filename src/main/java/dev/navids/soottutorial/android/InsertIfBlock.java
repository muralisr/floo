...
public class InsertIfBlock {
    // cmd line way to invoke this: ./gradlew run --args="InsertIfBlock file_path.apk"
    ...
    public static void main(String[] args){
        ...
        if (args.length < 1) {
            System.out.println("I need one argument after InsertIfBlock with the path to the apk file.\n./gradlew run --args=\"InsertIfBlock path_to_apk_file\"");
            return;
        }
        apkPath = args[0];
        // Initialize Soot
        InstrumentUtil.setupSoot(androidJar, apkPath, outputPath);
        InsertIfBlock a = new InsertIfBlock();
